  ...
  if (error) {
    return (
      <div className="flex h-dvh flex-col items-center justify-center bg-[#FDFCFB] px-10 text-center">
        ...
        <h2 className="mt-6 font-heading text-[18px] font-extrabold text-ink">Pelacakan Belum Tersedia</h2>
      ...
      <main className="px-5 pt-6">
        ...
        <div className="rounded-[28px] bg-white p-6 shadow-[0_8px_30px_rgba(0,0,0,0.04)] border border-stone-2">
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <h3 className="font-heading text-[16px] font-extrabold text-ink">
                {trackingData?.courier?.company?.toUpperCase()} - {trackingData?.courier?.type?.toUpperCase()}
              </h3>
            </div>
          </div>
          ...
          <div className="mt-6 flex items-center justify-between rounded-2xl bg-stone-1 px-4 py-3 border border-stone-2/50">
            <div>
              <p className="text-[10px] font-bold text-ink-4 uppercase tracking-tighter">Nomor Resi</p>
        ...
        {/* Tracking Timeline */}
        <div className="mt-8">
          <h3 className="px-1 font-heading text-[16px] font-extrabold text-ink mb-6">Status Pengiriman</h3>
          ...
          <div className="relative space-y-8 pl-8 before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[2px] before:bg-stone-2">
            {history.length === 0 ? (
              ...
              return (
                ...
                >
                  {/* Dot */}
                  <div className={`absolute -left-[30px] top-1 h-[24px] w-[24px] rounded-full border-4 border-[#FDFCFB] flex items-center justify-center shadow-sm ${
                    isLatest ? 'bg-primary text-white' : 'bg-stone-3 text-white'
                  }`}>
                    {isLatest ? <CheckCircle2 size={12} /> : <div className="h-1.5 w-1.5 rounded-full bg-white" />}
                  </div>

                  <div className={`rounded-2xl p-4 transition-all ${
                    isLatest ? 'bg-white shadow-[0_4px_20px_rgba(224,123,57,0.1)] border border-primary/20' : 'bg-stone-1/50 border border-transparent'
                  }`}>
                    <div className="flex justify-between items-start gap-2">
                      <p className={`font-heading text-[14px] font-extrabold leading-tight ${
                        isLatest ? 'text-ink' : 'text-ink-3'
                      }`}>
                        {step.status}
                      </p>
                      <p className="shrink-0 text-[10px] font-bold text-ink-4">
                        {step.updated_at ? new Intl.DateTimeFormat('id-ID', {
                          hour: '2-digit',
                          minute: '2-digit'
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-4">
              ...
              <div>
                ...
                <p className="text-[13px] font-bold line-clamp-1">{trackingData?.destination?.address || 'Alamat Penerima'}</p>
              </div>
            </div>
          </div>
        </div>
